...
main() {
  ...
  for i in $(seq 0 $((MESSAGE_COUNT - 1))); do
    ...
    if [ "$HAS_REACTION" = "false" ] && [ "$HAS_REPLY" = "false" ]; then
      ...
      # Use `cut -c` (character-based) instead of `head -c` (byte-based) to avoid
      # corrupting multibyte characters such as Japanese (3 bytes per char in UTF-8).
      if [ -n "$MESSAGE_TEXT" ] && [ "$MESSAGE_TEXT" != "null" ]; then
        MESSAGE_PREVIEW=$(echo "$MESSAGE_TEXT" | cut -c 1-50 | tr '\n' ' ' | sed 's/^[[:space:]]*//' | sed 's/[[:space:]]*$//')
      elif [ -n "$MESSAGE_BLOCK_TEXT" ] && [ "$MESSAGE_BLOCK_TEXT" != "null" ]; then
        MESSAGE_PREVIEW=$(echo "$MESSAGE_BLOCK_TEXT" | cut -c 1-50 | tr '\n' ' ' | sed 's/^[[:space:]]*//' | sed 's/[[:space:]]*$//')
      else
        MESSAGE_PREVIEW=""
      fi
